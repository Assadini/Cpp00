...
void addContact(PhoneBook &phonebook) {
    ...
    for (size_t i = 0; i < firstname.length(); i++) {
        if (std::isdigit(firstname[i])) {
            std::cout << "First name must not contain digits. Aborting.\n";
            return;
        }
    }
    ...
    for (size_t i = 0; i < lastname.length(); i++) {
        if (std::isdigit(lastname[i])) {
            std::cout << "Last name must not contain digits. Aborting.\n";
            return;
        }
    }
